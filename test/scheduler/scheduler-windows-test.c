...
int main(int argc, char **argv)
{
	...
	/* MMCSS mode must succeed even when every CPU placement API is absent. */
	reset();
	placement_enabled = false;
	api.topology = NULL;
	api.select = NULL;
	api.selected = NULL;
	api.process_selected = NULL;
	struct os_thread_scheduler *mmcss_only = os_thread_scheduler_begin(OS_THREAD_ROLE_AUDIO);
	CHECK(mmcss_only && mmcss_active && power_active && !cpu_active);
	os_thread_scheduler_end(mmcss_only);
	check_reverted();
